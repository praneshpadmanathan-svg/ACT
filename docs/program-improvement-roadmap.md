...
72. ~~Add questions to English's thinnest topics: dashes (2), colons (3), idioms (3), modifiers (4), fragments (4).~~ **Done.** English is 181 questions across 18 topics, thinnest topic 8. No topic is memorizable in a single drill session any more.
73. ~~Add questions to Math's thinnest topics: logarithms (2), matrices (2), complex numbers (2), systems (3).~~ **Done.** Math is 316 questions across 20 topics, thinnest topic 10. Every credited answer was recomputed from first principles by an independent verifier rather than trusted as authored.
74. ~~Treat Reading and Science as lower priority for raw count.~~ **Superseded.** Both were grown anyway while closing §76a: Reading is 163 questions across 10 topics and Science 176 across 9, each with a floor of 8. The bank totals 836 drill questions.
...
76. If content authoring is LLM-assisted (a reasonable way to close §72-73 at volume), treat the validation script as a hard gate on any batch-generated content — batch generation is exactly the scenario most likely to introduce duplicate IDs or malformed answer references.
    76a. ~~**Discovered by §39-44's own rule 4**: 8 zones teach a skill with no drill-bank topic of its own.~~ **Closed.** All 8 finer topics now have drill questions of their own — `text structure` (8), `tone` (8), `evidence` (9), `figurative language` (16), `units` (8), `rates` (8), `hypotheses` (10), `calculating from data` (8) — so the drill-bank taxonomy was widened to match the zones rather than the zones folded into coarse labels. The `NO_DRILL_TOPIC` exemption set is deleted and rule 4 now holds for all 37 zones with no carve-outs; `weakestTopics()` can see every landmark's skill. Four new passages were authored to support it: `sp20` (antacid dissolution, built for rates and unit conversion), `rp11` (a literary narrative written around locatable figures of speech), plus figurative-language items anchored on the existing `rp1` and `rp5` so the topic does not always draw the same text.
    76b. **Done, and hardened past the original plan.** `scripts/check-content.mjs` grew a rule 7 covering the defects batch authoring actually produced during §72-74: duplicate stems within one passage, textually identical choices, explanations naming a choice by letter (which rots the moment `shuffleChoices()` relabels them), unbalanced English guillemet spans, and Reading questions quoting phrases absent from their passage. Each sub-rule was negative-tested — a violation was injected and the build confirmed to fail — so none of them passes trivially.
...
## 19. Navigation — the rail

108. **Done.** The eight-item top bar became a left rail (`src/components/Shell.tsx`, `SideNav`). The row had run out of room — 815.9px of tabs inside a 1152px shell, which is why the wordmark had to stand down and why the rail could not turn on until `min-[1140px]` — and, more to the point, a row gives every destination identical weight and no space to say what it is for. The rail groups them under four headings (Your climb · Practice · Prove it · You), gives each a one-line hint, and shows the ones whose size is the selling point their own count. It turns on at `lg` (1024) now that it no longer competes with the page for horizontal width; below that a slim bar names where you are and a drawer carries the same groups.
109. The sliding underline is gone with it. It measured `offsetLeft` off the DOM and re-measured on route change, on `document.fonts.ready` and on `resize`; a vertical rail draws the active bar on the item's own leading edge, so there is nothing to measure and one less thing that can be wrong mid-animation.
110. The rail is `position: sticky`, never `fixed`, because it renders inside the shake stage in `App.tsx` and a transformed ancestor becomes the containing block for `position: fixed` — the same rule `RegionBackdrop` and `StoryOverlay` follow by sitting outside the stage instead.
111. The bank total sits at the foot of the rail and at the head of the landing page, in display type at a size nothing near it comes close to. It is the reason to open the app and it stopped being persuasive when it was sized like a badge in a row of four. The landing page's "skill zones" stat is gone — it quoted a number the flattened UI never shows anywhere.

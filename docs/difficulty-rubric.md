# What easy, medium and hard mean in the question bank

Every drill item carries a `difficulty` of `easy`, `medium` or `hard`. For most
of the bank's life those labels were assigned per batch, by whoever was writing
that batch, against nothing. The result was what you would expect: identical
structures carried different labels in different places. `neither/nor` was
`medium` in one item and `hard` in four others. A subject sitting directly next
to its verb was labelled `hard`. Thirty-eight English items were re-graded
against this document in one pass; write new items to it so that does not
happen again.

The label matters more than it looks. It drives which items a student sees as
they progress, and it is the only signal the app has for whether a section is
actually getting harder. A bank whose `hard` items are not hard teaches a
student that they are ready when they are not.

## The principle

**Difficulty is a property of the structure the item tests, not of how long the
sentence is or how unusual the vocabulary is.**

- **Easy** — the rule is visible at the point of use. Nothing stands between the
  student and the thing being tested, and the sentence does not pull toward a
  wrong answer.
- **Medium** — the rule has to be carried across something: an intervening
  phrase, a clause, a second sentence, one step of arithmetic.
- **Hard** — the sentence or figure actively pulls toward a wrong answer. A
  plausible wrong reading is available and costs nothing to take.

A long sentence with an obvious answer is easy. A short sentence with a live
trap is hard.
...
correct, and there is no easy version of that question. A handful of `easy` ones
exist where the passage answers the question in the next sentence; do not
manufacture more.

## Math

- **Easy** — one step, and the step is the named skill.
- **Medium** — two steps, or one step plus a setup the student has to build
  (drawing the triangle, naming the variable).
- **Hard** — three or more steps, a step that is not the named skill, or a
  standard wrong first move that produces one of the offered answers.

An item is not hard because the numbers are ugly. It is hard because a student
who understands the skill can still get it wrong.
...
Before labelling an item `hard`, name the wrong answer a competent student
would give, and say why the sentence or the figure invites it. If you cannot,
the item is `medium`.

Before labelling an item `easy`, check that nothing stands between the student
and the rule. An easy item is not a hard item with simpler words.
